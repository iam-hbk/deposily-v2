...
const apiAddClientSchema = insertClientSchema
  .omit({
  ...
  try {
    ...
    const existingClient = await db.query.clients.findFirst({
      where: and(
        ...
        eq(clients.clientReference, clientReference),
      ),
    });

    if (existingClient) {
      return NextResponse.json(
        { error: "Client reference already exists for this user." },
        { status: 409 },
      ); // 409 Conflict
    }
    ...
    const newClientData = {
      ...
      expectedPaymentDay:
        typeof expectedPaymentDay === "number"
          ? expectedPaymentDay.toString()
          : null,
    };
    ...
    if (error instanceof Error && error.message.includes("unique constraint")) {
      return NextResponse.json(
        {
          error:
            "A client with this reference might already exist or another unique constraint was violated.",
        },
        { status: 409 },
      );
    }
